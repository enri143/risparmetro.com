...
> **Ultimo aggiornamento: 23 giugno 2026** (sessione — C13 completo, D16/D17, E21 prep, push a `c7c7b9a`).
...
- **Test suite**: `79 passed · 3 skipped · 0 failed` (`npm run test`).
...
- **origin/main**: allineato a `c7c7b9a` (push 23 giu).
- ⚠️ **AZIONE PENDENTE**: ruotare `service_role` key del progetto Supabase `yrwjsztbibhnrlbvkayh` — `.env.test` era in commit `2a967c0` ora pubblico. Supabase Dashboard → Settings → API → Regenerate service_role.
...
- **D16** ✅ Osservabilità (Sentry) — errors-only, no replay, scrub provvigioni/segreti, no-op senza VITE_SENTRY_DSN
...
- **E20** ⏳ 💰 Billing · **E21** 🟡 Deploy prod + dominio (prep fatto, manca deploy) · **E22** ⏳ Tenant 0 = lucegas · **E23** ⏳ Primo broker esterno
  - Vercel: import repo, framework = Vite (auto), output `dist` (auto).
  - Env Vercel (Production): `VITE_SUPABASE_URL` + `VITE_SUPABASE_PUBLISHABLE_KEY` del progetto PROD `yrwjsztbibhnrlbvkayh` — MAI service_role, MAI progetto test. `VITE_SENTRY_DSN` opzionale.
  - Post-deploy: testare deep-link `/board` refresh → no 404 (verifica rewrite).
  - Dominio: collegare dopo primo deploy verde.
...
| 42 | D17: rimosse orfane board-storico (edge) + logo-new.png (asset); security: untrack .env.test + .gitignore esteso a .env.* | `2a967c0`+`0378605` | build OK, 73/3/0 |
| 43 | D16: Sentry init errors-only (no replay, scrub provvigioni/segreti) + ErrorBoundary in main.tsx + 6 test scrub | `d6414fe` | build OK, 79/3/0 |
| 44 | E21 prep: vercel.json SPA rewrite + .env.example (no valori) + .gitignore negazione .env.example | `c7c7b9a` | build OK, 79/3/0 |
...
- Sentry attivo solo con `VITE_SENTRY_DSN` settato; no-op altrimenti. No Session Replay per invariante provvigioni. Source maps + DSN su Vercel = follow-up E21.
- knip NON rileva l'uso delle edge function (invocate per stringa via `supabase.functions.invoke("nome")`): le considera tutte orfane. Lo stato orfano di un'edge si verifica SOLO con `grep -rn "nome-funzione" src/`, mai con knip. Attive confermate: analyze-cte (UploadPdfFlow), extract-bolletta-board, provision-tenant-user.
