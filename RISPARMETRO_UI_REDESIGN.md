...
- (WU5-prep) Stitch MCP non disponibile (timeout ripetuti, 0 schermate generate). Decisione: saltare la fase di reference visiva via Stitch; costruire direttamente 3 varianti React statiche (`OfferteDirezione1/2/3`) con dati fittizi in una route/tab di staging per confronto su iPad reale. Approccio più veloce e già in formato Tailwind/React consumabile. Le 3 direzioni: (1) righe orizzontali leaderboard full-width, risparmio a destra dominante; (2) griglia di carte con gerarchia verticale forte; (3) winner hero + inseguitori compatti sotto. Scelta finale dopo revisione visiva browser.
